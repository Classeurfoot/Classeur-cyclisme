...
def afficher_resultats(df_res):
    ...
    # On met à jour dynamic_cols pour ne pas bloquer les colonnes qu'on vient de supprimer
    dynamic_cols = [c for c in cols_cat if c in df_disp.columns]
    
    ed_df = st.data_editor(df_disp, column_config={"🛒": st.column_config.CheckboxColumn("Ajouter")}, disabled=dynamic_cols, hide_index=True, use_container_width=True)
